...
class DataExplorerApp(QMainWindow):
    def __init__(self):
        ...
        self.app_settings = {
            "compress": True, "theme": "Light", "font_size": 10, "default_engine": "Polars"
        }
    ...
    def setup_cli_tab(self):
        ...
        self.cli_output.setStyleSheet("font-family: 'Consolas', monospace; font-size: 10pt; background-color: #f7f7f7;")
        layout.addWidget(self.cli_output)
        
        input_layout = QHBoxLayout()
        self.cli_input = QLineEdit()
        self.cli_input.setPlaceholderText("명령어 입력 (예: /help, self.df.head())")
        self.cli_input.setStyleSheet("font-family: 'Consolas', monospace; height: 32px; background-color: #ffffff;")
    ...
    def smart_save_project(self, path):
        ...
        success, msg = SessionManager.save_project(path, self.variables, ui_state, compress=self.app_settings["compress"])
    ...
    def execute_cli(self):
        text = self.cli_input.text()
        if not text: return
        self.cli_output.append(f"<b style='color: #2e7d32;'>>>> {text}</b>")
        
        # Use REPL Handler
        res = self.repl.process_input(text)
    ...
    def load_data_async(self):
        ...
        if file_path: self.load_data_file_async(file_path)
    ...
    def generate_plot_dispatch(self):
        ...
        x = self.combo_x.currentText(); y = self.combo_y.currentText()
        sample_df = self.df if len(self.df) < 100000 else self.df.sample(100000)
        ...
        if self.viz_lib == "Plotly":
            ...
            def _mpl_task(): return VizManager.generate_matplotlib_fig(sample_df, plot_type, x, y)
    ...
    def run_engine_ops(self):
        if self.df.empty: return
        rows_str = self.input_loc_rows.text(); cols_str = self.input_loc_cols.text()
        query_str = self.input_query.text(); new_var_name = self.input_new_var.text()
        def _task():
            res_df = self.df
            if rows_str != ":" or cols_str != ":":
                res_df = res_df.loc[eval(rows_str) if rows_str != ":" else slice(None), 
                                    eval(cols_str) if cols_str != ":" else slice(None)]
            if query_str: res_df = res_df.query(query_str)
            return res_df
        def _on_success(res):
            self.variables[new_var_name] = res; self.update_explorer()
            QMessageBox.information(self, "성공", f"연산 완료: {new_var_name}")
