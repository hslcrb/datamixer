...
class MiniBrowser(QWidget):
    """Integrated Premium Web Browser with Persistence."""
    def __init__(self, parent=None):
        ...
        self.btn_back = QPushButton("<"); self.btn_forward = QPushButton(">"); self.btn_reload = QPushButton("R")
        self.url_bar = QLineEdit(); self.url_bar.setPlaceholderText("검색어나 URL..."); self.url_bar.returnPressed.connect(self.navigate_to_url)
        self.btn_go = QPushButton("GO"); self.btn_go.clicked.connect(self.navigate_to_url)
        ...
        self.nav_layout.addWidget(self.url_bar, 1); self.nav_layout.addWidget(self.btn_go); self.nav_layout.addWidget(self.btn_ext)
        self.layout.addLayout(self.nav_layout)
        self.browser = QWebEngineView(); self.page = QWebEnginePage(self.profile, self.browser); self.browser.setPage(self.page)
        self.browser.setUrl(QUrl("https://www.google.com")); self.layout.addWidget(self.browser)
        self.btn_back.clicked.connect(self.browser.back); self.btn_forward.clicked.connect(self.browser.forward); self.btn_reload.clicked.connect(self.browser.reload)
...
class DataExplorerApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Datamixer Enterprise - Ultimate Multi-Engine Hub V7")
        ...
        self.status_label.setText("Enterprise AI HUB V7 - Multi-Engine Stack Online (Pd/Pl/Np)")
    ...
    def setup_docks(self):
        # Explorer (Now with Type)
        self.explorer_dock = QDockWidget("변수 탐색기 (Multi-Engine)", self)
        self.explorer_tree = QTreeWidget(); self.explorer_tree.setHeaderLabels(["변수명", "라이브러리/형태", "메모리 점유"])
        ...
        # AI Insight
        self.insight_dock = QDockWidget("AI 인사이트 리포트 (통계)", self)
        self.insight_output = QTextEdit(); self.insight_output.setReadOnly(True); self.insight_output.setStyleSheet("font-family: 'Cascadia Code'; background-color: #1a1b26;"); self.insight_dock.setWidget(self.insight_output); self.addDockWidget(Qt.RightDockWidgetArea, self.insight_dock)

        # Jupyter Console
        self.console_dock = QDockWidget("Jupyter Lab Console", self)
        self.console_dock.setWidget(self.jupyter_console.widget); self.addDockWidget(Qt.BottomDockWidgetArea, self.console_dock)
        
        # [NEW] Engine Control Toolbox
        self.props_dock = QDockWidget("데이터 워크벤치 설정", self)
    ...
    def setup_props_panel(self):
        ...
        core_group = QGroupBox("병렬 엔진 매핑 (Pd/Pl/Np)")
        cl = QFormLayout(core_group)
        self.combo_engine = QComboBox(); self.combo_engine.addItems(["Polars (Blazing Fast)", "Pandas (Feature Rich)"])
        cl.addRow("코어 처리 엔진:", self.combo_engine)
        self.lbl_data_info = QLabel("데이터 대기..."); self.lbl_data_info.setWordWrap(True); self.lbl_data_info.setStyleSheet("color: #7aa2f7;")
        cl.addRow(self.lbl_data_info); layout.addWidget(core_group)

        viz_group = QGroupBox("시각화 컨트롤"); vfl = QFormLayout(viz_group); self.combo_plot_type = QComboBox()
        self.combo_plot_type.addItems(["히스토그램", "산점도", "박스 플롯", "히트맵", "선 그래프"]); vfl.addRow("그래프:", self.combo_plot_type)
        self.combo_x = QComboBox(); self.combo_y = QComboBox(); vfl.addRow("X:", self.combo_x); vfl.addRow("Y:", self.combo_y); self.btn_plot = QPushButton("엔진 실행"); self.btn_plot.clicked.connect(self.generate_plot_dispatch); vfl.addRow(self.btn_plot); layout.addWidget(viz_group); layout.addStretch()
    ...
    def load_data_file_async(self, p):
        def _task():
            ...
            # If engine is polars, convert for internal pd-compatible display if needed 
            # (but we can support both in vars)
            return df, m, enc if s else None
        def _ok(res):
            if not res: return
            df, m, enc = res; self.df = df; vn = os.path.basename(p).replace(".", "_"); self.variables[vn] = df; self.update_explorer(); self.update_table(); self.update_viz_combos(); self.display_data_mapping(df, enc)
    ...
    def on_intelligence_finished(self, r):
        txt = "<b>[AI 허브 V7]</b><br><br>"; [txt := txt + f"▶ {i}<br>" for i in r["insights"]]
        self.insight_output.setHtml(txt)
        if r["suggestions"]:
            b = r["suggestions"][0]; self.combo_plot_type.setCurrentText(b["type"]); self.combo_x.setCurrentText(b["x"])
            if b["y"]: self.combo_y.setCurrentText(b["y"]); self.generate_plot_dispatch()
    ...
    def display_data_mapping(self, df, e):
        tip = "자동 감지 인코딩"
        i = f"<b>Encoding:</b> {e} [<b>?</b>]<br><b>Shape:</b> {df.shape[0]:,} x {df.shape[1]:,}<br><b>Schema:</b><br>"; [i := i + f"- {c}: {str(df[c].dtype)}<br>" for c in df.columns]; self.lbl_data_info.setText(i); self.lbl_data_info.setToolTip(f"<b>[Help]</b><br>{tip}")

    def load_data_async(self):
        p, _ = QFileDialog.getOpenFileName(self, "Load", "", "Data (*.csv *.xlsx *.parquet)"); [self.load_data_file_async(p) if p else None]
    ...
    def update_table(self): 
        if not self.df.empty:
            # Table is currently pandas-based, we convert if it's polars
    ...
    def update_explorer(self): 
        ...
        for v, d in self.variables.items():
            ...
            size = f"{d.shape[0]}x{d.shape[1]}" if hasattr(d, 'shape') else ("List/Other")
            mem = f"{d.memory_usage(deep=True).sum()/1024**2:.2f}MB" if hasattr(d, 'memory_usage') else "N/A"
            self.explorer_tree.addTopLevelItem(QTreeWidgetItem([v, f"{dtype} ({size})", mem]))
